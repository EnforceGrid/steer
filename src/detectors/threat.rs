...
static REGEX_SET: Lazy<RegexSet> =
    Lazy::new(|| RegexSet::new(PATTERNS.iter()).expect("threat patterns must compile"));
...
mod tests {
    ...
    fn blackmail_coercion() {
        let r = detector()
            .scan("I will release your photos or I'll expose your secrets unless you pay");
        assert!(r.detected);
