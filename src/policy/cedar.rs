...
mod tests {
    ...
    // ── pii_findings set-based context tests ────────────────────────────────
    // Cover the v0.1.0 architectural fix: instead of routing block decisions
    // through a single `pii_detected` boolean, policies enumerate the
    // pattern names they care about via `context.pii_findings.containsAny`.
    // This lets the shipped `default-secrets-block` rule block on auth-secret
    // patterns while `default-pii-flag` only flags broad personal data —
    // the distinction the YAML `pii.patterns` list always implied.

    #[test]
    fn pii_findings_set_blocks_on_secret_pattern() {
        let policy = r#"
            permit(principal, action, resource);
            ...
            when {
              context.pii_findings.containsAny([
                "openai_key", "anthropic_key", "aws_access_key"
              ])
            };
        "#;
        let d = eval(
            policy,
            &json!({
                "pii_findings": ["openai_key"],
                "pii_detected": true,
            }),
        );
        assert_eq!(d.action, EnforcementAction::Block);
        assert_eq!(d.rule_id.as_deref(), Some("default-secrets-block"));
    }

    #[test]
    fn pii_findings_set_does_not_block_on_personal_data() {
        // Bare email/SSN should not trip the secrets-block rule. Customer
        // support and other legitimate flows must keep working.
        let policy = r#"
            permit(principal, action, resource);
            @id("default-secrets-block")
            @enforcement("block")
    ...
    #[test]
    fn pii_findings_flag_personal_data_distinct_from_block_secrets() {
        // Both shipped rules loaded at once. Email should flag, OpenAI key
        // should block, simultaneous findings should block (block > flag).
        let policy = r#"
            permit(principal, action, resource);
            @id("default-pii-flag")
            @enforcement("flag")
            forbid(principal, action, resource)
            when {
              context.pii_findings.containsAny(["credit_card","ssn","email","phone"])
            };
            @id("default-secrets-block")
            @enforcement("block")
            forbid(principal, action, resource)
            when {
              context.pii_findings.containsAny(["openai_key","anthropic_key","aws_access_key"])
            };
        "#;
        ...
        let d_email = eval(
            policy,
            &json!({"pii_findings": ["email"], "pii_detected": true}),
        );
        assert_eq!(d_email.action, EnforcementAction::Flag);

        // Secret-only → block
        let d_key = eval(
            policy,
            &json!({"pii_findings": ["openai_key"], "pii_detected": true}),
        );
        assert_eq!(d_key.action, EnforcementAction::Block);

        // Both → block wins
        let d_both = eval(
            policy,
            &json!({"pii_findings": ["email", "openai_key"], "pii_detected": true}),
        );
        assert_eq!(d_both.action, EnforcementAction::Block);
        assert_eq!(d_both.rule_id.as_deref(), Some("default-secrets-block"));

        // Neither → allow
        let d_none = eval(policy, &json!({"pii_findings": [], "pii_detected": false}));
        assert_eq!(d_none.action, EnforcementAction::Allow);
    }

    #[test]
    fn pii_findings_empty_set_passes_through() {
        // Empty pii_findings array must not match any containsAny call.
        let policy = r#"
            permit(principal, action, resource);
            @id("default-secrets-block")
            @enforcement("block")
            forbid(principal, action, resource)
            when {
              context.pii_findings.containsAny(["openai_key"])
            };
        "#;
        let d = eval(policy, &json!({"pii_findings": [], "pii_detected": false}));
        assert_eq!(d.action, EnforcementAction::Allow);
    }

    #[test]
    fn pii_findings_observation_mode_rewrites_secrets_block_to_flag() {
        // policy.mode: observe should rewrite the block rule to flag so the
        // secret-leak event is logged but not blocked. Exercises the
        // load-time rewrite + cedar evaluation end-to-end.
        let input = r#"
            permit(principal, action, resource);
            @id("default-secrets-block")
            @enforcement("block")
