...
mod tests {
    ...
    fn bedrock_corrupt_byte_drained_valid_frame_recovered() {
        ...
        assert_eq!(
            frames.len(),
            1,
            "valid frame should be recovered after corrupt byte is drained"
        );
        assert!(buf.is_empty());
    }
    ...
    fn detect_provider_explicit_name_wins() {
        assert_eq!(
            detect_provider(Some("bedrock"), "/v1/chat/completions", None),
            "bedrock"
        );
        assert_eq!(
            detect_provider(Some("anthropic"), "/v1/chat/completions", None),
            "anthropic"
        );
        assert_eq!(
            detect_provider(Some("gemini"), "/v1/chat/completions", None),
            "gemini"
        );
        assert_eq!(
            detect_provider(Some("openai"), "/v1/messages", None),
            "openai"
        );
    }
    ...
    fn detect_provider_falls_back_to_path() {
        ...
        assert_eq!(
            detect_provider(None, "/v1beta/models/gemini/generateContent", None),
            "gemini"
        );
        assert_eq!(detect_provider(None, "/bedrock/invoke", None), "bedrock");
        assert_eq!(
            detect_provider(None, "/v1/chat/completions", None),
            "openai"
        );
    }
    ...
    fn detect_provider_unknown_explicit_falls_back_to_path() {
        ...
        assert_eq!(
            detect_provider(Some("unknown_provider"), "/v1/messages", None),
            "anthropic"
        );
    }
}
