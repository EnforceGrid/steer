...
fn parse_header_string(header_bytes: &[u8], target_name: &str) -> Option<String> {
    ...
    while pos < header_bytes.len() {
        ...
        pos += 1;
        if pos + name_len > header_bytes.len() { break; }
        let name = &header_bytes[pos..pos + name_len];
        pos += name_len;
        if pos >= header_bytes.len() { break; }
        ...
        let value_size: Option<usize> = match type_code {
            ...
            9     => Some(16),        // uuid
            6 | 7 => {               // bytes / string: 2-byte length prefix
                if pos + 2 > header_bytes.len() { break; }
        ...
        if pos + size > header_bytes.len() { break; }
...
mod tests {
    ...
    fn parse_frame_detects_exception_frame() {
        ...
        let frames = parser().parse_frame(&raw);
        assert_eq!(frames.len(), 1);
        assert!(frames[0].is_error, "exception frames must set is_error = true");
        assert!(!frames[0].is_done);
        assert!(frames[0].data.contains("ThrottlingException") || frames[0].data.contains("Rate exceeded"));
    }
    ...
    fn parse_frame_error_on_headers_overflow() {
        ...
        assert!(frames.iter().all(|f| f.is_error) || frames.is_empty() || frames.len() >= 1);
    }
    ...
    fn encode_text_delta_produces_valid_binary_frame() {
        ...
        assert!(!frames[0].is_error, "CRC must be valid — got error: {:?}", frames[0].data);
        assert!(!frames[0].is_done);
    }
    ...
    fn encode_text_delta_crc_is_correct() {
        ...
        let total_length = u32::from_be_bytes([encoded[0], encoded[1], encoded[2], encoded[3]]) as usize;
        assert_eq!(total_length, encoded.len(), "total_length must match actual frame size");

        let prelude_crc_stored = u32::from_be_bytes([encoded[8], encoded[9], encoded[10], encoded[11]]);
        let prelude_crc_computed = crc32fast::hash(&encoded[0..8]);
        assert_eq!(prelude_crc_stored, prelude_crc_computed, "prelude CRC must be valid");

        let msg_crc_stored = u32::from_be_bytes([
            encoded[total_length - 4], encoded[total_length - 3],
            encoded[total_length - 2], encoded[total_length - 1],
        ]);
        let msg_crc_computed = crc32fast::hash(&encoded[0..total_length - 4]);
        assert_eq!(msg_crc_stored, msg_crc_computed, "message CRC must be valid");
    }
    ...
    fn parse_header_string_handles_non_string_type_before_target() {
        ...
        assert_eq!(result.as_deref(), Some("content_block_delta"),
