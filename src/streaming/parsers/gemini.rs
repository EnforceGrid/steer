...
mod tests {
    ...
    fn parse_frame_handles_empty_candidates_gracefully() {
        ...
        assert!(
            frames[0].is_done,
            "promptFeedback.blockReason should mark stream as done"
        );
    }
    ...
    fn parse_frame_unspecified_finish_reason_is_not_done() {
        ...
        assert!(
            !frames[0].is_done,
            "UNSPECIFIED finishReason should not mark stream done"
        );
    }
    ...
    fn parse_frame_raw_is_per_line_not_whole_chunk() {
        ...
        assert!(
            raw0.contains("\"a\"") && !raw0.contains("\"b\""),
            "first frame raw should only contain first line"
        );
        assert!(
            raw1.contains("\"b\"") && !raw1.contains("\"a\""),
            "second frame raw should only contain second line"
        );
    }
    ...
    fn encode_text_delta_no_finish_reason() {
        ...
        assert!(
            !frames[0].is_done,
            "text delta without finishReason must not be done"
        );
    }
    ...
    fn encode_text_delta_special_chars_json_escaped() {
        ...
        assert_eq!(frames.len(), 1);
        let v: serde_json::Value = serde_json::from_str(&frames[0].data).unwrap();
        let recovered = v["candidates"][0]["content"]["parts"][0]["text"]
            .as_str()
            .unwrap();
